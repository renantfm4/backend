...
class User(AuditMixin, Base):
    ...
    unidadeSaude = relationship(
        "UnidadeSaude", secondary=user_unidadeSaude, back_populates="users"
    )
...
class UnidadeSaude(AuditMixin, Base):
    ...
    users = relationship(
        "User", secondary=user_unidadeSaude, back_populates="unidadeSaude"
    )
...
class Atendimento(AuditMixin, Base):
    ...
    investigacao_lesoes_suspeitas_id = Column(
        Integer, ForeignKey("investigacaoLesoesSuspeitas.id")
    )
...
class SaudeGeral(Base):
    ...
    frequencia_atividade_fisica = Column(
        Enum(
            "Diária",
            "Frequente",
            "Moderada",
            "Ocasional",
            name="frequencia_atividade_fisica_enum",
        ),
        default=None,
    )
...
class AvaliacaoFototipo(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            quantidade_sardas.in_([0, 1, 2, 3]), name="check_quantidade_sardas"
        ),
        ...
        CheckConstraint(
            sensibilidade_solar.in_([0, 1, 2, 3, 4]), name="check_sensibilidade_solar"
        ),
    )
...
class HistoricoCancerPele(Base):
    ...
    grau_parentesco = Column(
        Enum(
            "Pai", "Mãe", "Avô/Avó", "Irmão/Irmã", "Outro", name="grau_parentesco_enum"
        ),
        nullable=True,
    )
    tipo_cancer_familiar = Column(
        Enum(
            "Melanoma",
            "Carcinoma Basocelular",
            "Carcinoma Espinocelular",
            "Outro",
            name="tipo_cancer_familiar_enum",
        ),
        nullable=True,
    )
    ...
    tipo_cancer_pessoal = Column(
        Enum(
            "Melanoma",
            "Carcinoma Basocelular",
            "Carcinoma Espinocelular",
            "Outro",
            name="tipo_cancer_pessoal_enum",
        ),
        nullable=True,
    )
...
class FatoresRiscoProtecao(Base):
    ...
    frequencia_exposicao_solar = Column(
        Enum(
            "Diariamente",
            "Algumas vezes por semana",
            "Ocasionalmente",
            name="frequencia_exposicao_enum",
        ),
        nullable=True,
    )

    queimaduras_graves = Column(Boolean, nullable=False, default=False)
    quantidade_queimaduras = Column(
        Enum("1-2", "3-5", "Mais de 5", name="quantidade_queimaduras_enum"),
        nullable=True,
    )

    uso_protetor_solar = Column(Boolean, nullable=False, default=False)
    fator_protecao_solar = Column(
        Enum("15", "30", "50", "70", "100 ou mais", name="fator_protecao_enum"),
        nullable=True,
    )
    ...
    frequencia_checkups = Column(
        Enum("Anualmente", "A cada 6 meses", "Outro", name="frequencia_checkups_enum"),
        nullable=True,
    )
...
class InvestigacaoLesoesSuspeitas(Base):
    ...
    tempo_alteracoes = Column(
        Enum(
            "Menos de 1 mês",
            "1-3 meses",
            "3-6 meses",
            "Mais de 6 meses",
            name="tempo_alteracoes_enum",
        ),
        nullable=True,
    )
