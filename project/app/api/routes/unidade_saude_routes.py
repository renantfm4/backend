...
async def cadastrar_unidade_saude(
    ...
    stmt = select(models.UnidadeSaude).filter(
        models.UnidadeSaude.codigo_unidade_saude == unidade_data.codigo_unidade_saude
    )
...
@router.get(
    "/listar-usuarios-unidade-saude/{unidade_id}",
...
async def listar_usuarios_unidade_saude(
    ...
    stmt = (
        ...
        .filter(models.UnidadeSaude.id == unidade_id)
        .options(
            selectinload(models.UnidadeSaude.users).selectinload(models.User.roles)
        )
    )
    result = await db.execute(stmt)
    unidade = result.scalars().first()
    ...
    for user in unidade.users:
        ...
        nivel_acesso = (
            max([role.nivel_acesso for role in user.roles]) if user.roles else None
        )
